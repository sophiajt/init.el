...
(use-package org-roam
  ...
  :custom
  (org-roam-directory (file-truename "/home/jt/Source/org-roam"))
...
(defun fzf-directory-jt ()
  ...
  (interactive)
   (let ((path (locate-dominating-file default-directory ".git")))
    ...
    (fzf/start path 
               (lambda (x)
                 (let ((f (expand-file-name x path)))
                   (when (file-exists-p f)
                     (find-file f)))))
      (user-error "Not inside a Git repository"))))

(defun rg-directory-jt ()
  "Starts an rg session at the specified directory."
  (interactive)
   (let ((path (locate-dominating-file default-directory ".git")))
    (if path
	(rg-run (rg-read-pattern "literal") "*.*" path)
      (user-error "Not inside a Git repository"))))

; Custom keybindings
(global-set-key (kbd "TAB") 'lsp-format-buffer)
(global-set-key (kbd "C-/") 'comment-line)
(global-set-key (kbd "C-p") 'fzf-directory-jt)
(global-set-key (kbd "C-f") 'rg-directory-jt)
(global-set-key (kbd "C-r") 'lsp-rename)
(global-set-key (kbd "C-\\") 'lsp-ui-peek-find-references)
(global-set-key (kbd "C-e") 'lsp-execute-code-action)
(global-set-key (kbd "C-z") 'undo-tree-undo)
(global-set-key (kbd "C-t") 'lsp-goto-type-definition)
(global-set-key (kbd "<M-right>") (lambda () (interactive) (history-add-history) (lsp-find-definition)))
(global-set-key (kbd "<M-left>") 'history-prev-history)

;; Rust and lsp settings
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(lsp-ui cargo emojify ripgrep history smooth-scrolling company ## rustic lsp-mode)))
...
(setq-default mode-line-format '("%e" mode-line-front-space mode-line-mule-info mode-line-client mode-line-modified mode-line-remote mode-line-frame-identification mode-line-buffer-identification "   " mode-line-position mode-line-misc-info))
